...
**Отложено — верификационный гейт находок ревью (машинный «ловец» ложных critical).** LLM-ревьюер
периодически выдаёт уверенные ложные `[critical]` — напр. «SyntaxError: не закрыта кавычка» на
файле, который чисто компилируется (артефакт: `[REDACTED]` в diff читается моделью как оборванный
строковый литерал + неточная привязка к номерам строк). Такие заявки **тривиально проверяются
машиной**, поэтому по инварианту «ловца ошибок делаем машинным гейтом» стоит добавить в `review/`
дешёвый пост-фильтр: находку, утверждающую синтаксическую ошибку в файле из diff, прогнать через
`ast.parse`/`py_compile` (для др. языков — доступный парсер); **не воспроизводится → выкинуть или
понизить severity** (как downgrade critical→medium без воспроизведения в CLAUDE.md). Заодно
проверить, не редактится ли в diff безобидный синтетический «секрет» из теста — иначе ревью будет
регулярно спотыкаться на том же месте. Ведущий: `llm-engineer` (дизайн правила) + `backend-developer`
(реализация в `review/reviewer.py`), профиль P4, `security-auditor` обязателен (трогаем обработку
diff/редакцию).
