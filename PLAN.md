...
| **4 ⏸** | Рой агентов (Слой B) — **отложен** (решение 2026-07-20) | Задача-изменение → рой (planner/critic/coder/reviewer/judge) → подтверждение → PR |
...
> ✅ **ВЫПОЛНЕНО 2026-07-19 (авто + ручная dogfood прошли).** Тестовый PR #1 → Action → AI-комментарий
> с реальными багами и ссылками на RAG-контекст проекта (fix `711b898`: `gh pr *` через `--repo`, без
> checkout). Автоматизация ревью кода по запросу пользователя: GitHub Action на PR → backend
...
**Отложено — подавление ложных `[critical]` в AI-ревью (машинный «ловец» + хардненг промпта).**
LLM-ревьюер устойчиво выдаёт уверенные ложные `[critical]`. Диагностика PR #6 (2026-07-26): **все 3
крита в трёх push-комментариях ложные**, и бьют в **два корня** (НЕ чанкинг — ревью = один вызов на
весь diff):
- **Усечение diff (>200 КБ).** Модель видит обрубленный на лимите diff и принимает «diff кончился» за
  «код неполный» → ложный «SyntaxError, функция обрывается» (крит #1: `generator.py:L1-372` — файл
  ровно 372 строки, `py_compile` чист).
- **Маркер `[REDACTED]`.** Барьер секретов подставляет `[REDACTED]` в diff, модель читает это как
  оборванный литерал / некорректный файл (крит #2: `test_knowledge.py` «не закрыта кавычка» —
  компилируется; крит #3: `.env.example` «замените [REDACTED] на [REDACTED]» — заявка бессмысленна,
  `.env.example` полон `KEY=...`, их ест `redact`).

Фикс — **комплект из трёх частей** (`ast.parse`-гейт из первой редакции этого пункта закрывал только
SyntaxError-корень #1/#2, но НЕ #3):
1. **Хардненг `REVIEW_SYSTEM_PROMPT`:** явно — «`[REDACTED]` = наш маркер вырезанного секрета, НЕ
   исходный код; никогда не считать его синтаксической ошибкой, неполнотой или некорректностью файла;
   усечённый на лимите diff — не признак неполного кода».
2. **Верификационный гейт:** находка про SyntaxError/неполный код → `ast.parse`/`py_compile` реального
   файла из diff (для др. языков — доступный парсер); **не воспроизводится → выкинуть/понизить
   severity** (как downgrade critical→medium без воспроизведения в CLAUDE.md).
3. **Фильтр находок по evidence:** выкидывать находку, чья цитата-доказательство — по сути `[REDACTED]`
   (ревьюить там нечего). Заодно решить, исключать ли `.env.example`-подобные файлы из ревью целиком.

Ведущий: `llm-engineer` (дизайн правил+промпта) + `backend-developer` (реализация в
`review/reviewer.py`), профиль **P4**, `security-auditor` **обязателен** (трогаем обработку
diff/редакцию/промпт — security-gate).

---

## Этап 4 — Рой агентов (Слой B)

> ⏸ **МУЛЬТИРОЛЕВОЙ рой ОТЛОЖЕН (решение пользователя 2026-07-20).** MVP 0–3 + доработки задеплоены.
> ✅ **MVP-вход в Слой B РЕАЛИЗОВАН (учебное задание 3):** ОДИН файловый tool-агент (DeepSeek
> function-calling, `agent/tools.py`+`agent/loop.py`, tools search_code/read_file/list_files/
> propose_patch/write_file/finish, human-confirm PR через `api/agent.py`). Мультиролевой рой
> (planner/critic/coder/reviewer/judge) надстраивается позже над тем же `agent/tools.py`. Дизайн-
> гипотеза ролей ниже сохранена.
...
**Техдолг (уборка, не срочно).**
- 🧹 **Удалить мёртвый bulk-эндпоинт базы знаний.** После фичи ручной пометки «Изучено» (PR #10,
  2026-07-27) авто-пометка убрана, но на бэкенде остались неиспользуемые фронтом
  `POST /api/knowledge/projects/{id}/read` (`app/api/knowledge.py`) + `db.mark_concepts_known`
  (`app/db.py`), оставленные «для обратной совместимости». Bulk-mark-all противоречит ручной
  модели → удалить эндпоинт + функцию + их тесты (`test_api_knowledge` read-ветка,
  `test_knowledge::test_mark_concepts_known_idempotent`). Отмечено в ревью PR #10. Профиль P6/P1.
