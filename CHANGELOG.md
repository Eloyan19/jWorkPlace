...
## Реорганизация вкладок UI + уборка техдолга (НЕ задеплоено)

Автономный execution-loop по пулу [`TASKS.md`](./TASKS.md) (T01–T16, метрики — [`METRICS.md`](./METRICS.md));
цель — «раздутый» набор вкладок стал понятнее.
...
- Каждая фича — через PR (#13–#18) с QA-циклом (Level1 pytest+vitest, Level2 Playwright smoke);
