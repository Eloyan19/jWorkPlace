...
## Реорганизация вкладок UI + FAQ + уборка техдолга (deploy `964704e`)

Автономный execution-loop по пулу [`TASKS.md`](./TASKS.md) (T01–T16 + фича «Справка»; метрики —
[`METRICS.md`](./METRICS.md)); цель — «раздутый» набор вкладок стал понятнее. **Задеплоено на
`https://jwork.jorchik.com`** (health `version=964704e`); прод-бандл отдаёт новые вкладки.

- **UX вкладок.** Разведены **Чат vs Поиск** поясняющими подзаголовками (общий `PanelHeader`); вкладка
  **«Правки»** получила сегмент-переключатель «Быстрая правка» ↔ «Агент по файлам» (`EditsPanel`)
  вместо стопки; **«Поддержка»** переименована в **«Поддержка сервиса»** и визуально отделена как
  сервис-уровневая (не про репозиторий).
- **Поведение.** Персист активной вкладки в `localStorage` (не сбрасывается на reload); клавиатурная
  навигация таб-бара (←/→ кольцом, Home/End, roving tabindex); единый `EmptyState` для проектных
  вкладок без выбранного проекта; `ActiveProjectIndicator` — имя активного проекта над панелями.
- **Техдолг.** Удалён мёртвый bulk-эндпоинт `POST /api/knowledge/projects/{id}/read` +
  `db.mark_concepts_known` + их тесты (follow-up ревью PR#10; per-concept `/known` сохранён).
- **Тесты.** Зафиксированы набор/порядок вкладок и отделение «Поддержки»; юнит-тест `PanelHeader`;
  guard независимости `/api/support/ask` от проекта. Итог: **364 pytest + ~93 vitest** зелёные.
- **Вкладка «Справка» (FAQ, PR #19).** Статическая сервис-вкладка `FaqPanel`: краткое описание
  каждого режима + блок «Чат vs Поиск». Сервис-секция обобщена до `SERVICE_TABS=[faq, support]`
  (обе отделены `tab-service`, индикатор проекта на них скрыт).
- Каждая фича — через PR (#13–#19) с QA-циклом (Level1 pytest+vitest, Level2 Playwright smoke);
  прочее (тесты, рефактор, доки) — прямыми коммитами в `master`.
- **Deploy `964704e`:** `deploy/redeploy.sh all` (backend GIT_SHA+рестарт, фронт Vite-build →
  `/var/www/jworkplace`, nginx reload); health `{"status":"ok","version":"964704e"}`; прод-бандл
  содержит «Справка»/«Поддержка сервиса»/«без генерации»/«Быстрая правка»/«Агент по файлам».
