...
Сверху всегда — статус сервиса и список проектов (выбор активного проекта). Ниже — вкладки:

| Вкладка | Что делает |
|---|---|
| **Чат** | Задаёте вопрос по коду — LLM отвечает **прозой с цитатами** из проекта (grounded). |
| **О проекте** | Автоматическая выжимка о репозитории и его ключевых концептах. |
| **Структура** | Дерево файлов и символов проекта (без LLM). |
| **Поиск** | **Сырые** релевантные фрагменты кода со скорами — находит *места*, без генерации ответа. |
| **Правки** | Переключатель: «Быстрая правка» (одношаговый патч → PR) ↔ «Агент по файлам» (автономный многошаговый). |
| **Поддержка сервиса** | Вопросы о **самом jWorkPlace** (не о вашем репозитории) — стоит отдельно справа. |

**Чат vs Поиск:** Поиск = «где это в коде» (список фрагментов); Чат = «дай ответ» (связный текст LLM
поверх найденного). Проектные вкладки требуют выбранного проекта; «Поддержка сервиса» доступна всегда.
Активная вкладка запоминается между перезагрузками; таб-бар управляется стрелками ←/→ (a11y).
