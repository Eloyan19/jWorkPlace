...
Таб-бар — единый `nav[role=tablist]`; вкладки делятся на **проектные** и **сервисную**.
...
| Поддержка сервиса | `support` | `SupportPanel` | FAQ о **самом сервисе** jWorkPlace, не о репо — визуально отделена (`tab-support`, `margin-left:auto`) |

**Инварианты таб-бара:**
- Источник вкладок — массивы `PROJECT_TABS` + `SUPPORT_TAB` (единый `TABS`/`TAB_KEYS` для валидации).
...
- **Проектные** вкладки требуют активного проекта: без него рендерится `EmptyState` (панель не
  монтируется). **Сервисная** «Поддержка» доступна всегда (не читает `activeProject`).
