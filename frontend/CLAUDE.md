...
## Структура вкладок (`App.tsx`)

Таб-бар — единый `nav[role=tablist]`; вкладки делятся на **проектные** и **сервисные**.

| Вкладка (label) | key | Компонент | Назначение |
|---|---|---|---|
| Чат | `chat` | `ChatPanel` | grounded-ответ LLM по коду с цитатами (`/api/chat`) |
| О проекте | `summary` | `SummaryPanel` | выжимка-база знаний о репо (`/api/knowledge/...`) |
| Структура | `structure` | `StructurePanel` | дерево файлов+символов, без LLM |
| Поиск | `search` | `SearchPanel` | **сырые** релевантные фрагменты со скорами (`/api/search`), без генерации |
| Правки | `edits` | `EditsPanel` | обёртка с сегмент-переключателем: «Быстрая правка» (`EditPanel`) ↔ «Агент по файлам» (`AgentPanel`) |
| Справка | `faq` | `FaqPanel` | статическое описание каждой вкладки сервиса (без api) — визуально отделена (`tab-service`, `margin-left:auto`) |
| Поддержка сервиса | `support` | `SupportPanel` | FAQ о **самом сервисе** jWorkPlace, не о репо — визуально отделена (`tab-service`) |

**Инварианты таб-бара:**
- Источник вкладок — массивы `PROJECT_TABS` + `SERVICE_TABS` (единый `TABS`/`TAB_KEYS` для валидации).
- **Чат vs Поиск** — разное: Чат = ответ прозой от LLM; Поиск = выдача фрагментов без генерации.
  Разница подписана через `PanelHeader({title, subtitle})` у каждой панели — не убирать.
- **Проектные** вкладки требуют активного проекта: без него рендерится `EmptyState` (панель не
  монтируется). **Сервисные** («Справка», «Поддержка») доступны всегда (не читают `activeProject`).
- Активная вкладка персистится в `localStorage` (`activeTab.ts`, ключ `jwp_active_tab`, fallback `chat`).
- A11y: `role=tablist/tab/tabpanel` + `aria-selected`; клавиатура ←/→ (кольцо), Home/End, roving
  `tabIndex` (активный `0`, прочие `-1`), фокус следует за активной вкладкой.
- `ActiveProjectIndicator` над панелями показывает имя активного проекта на проектных вкладках.
- Панели не размонтируются при переключении вкладок (`hidden`) — состояние сохраняется (см. ниже).
