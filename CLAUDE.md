...
```
Браузер ──HTTPS──▶ nginx (домен jWorkPlace:443)
                     ├─ /                 → статика фронта
...
- **Домен:** куплен пользователем (уточнить и прописать при деплое).
...
- [ ] **Имя продукта и домен** — рабочее имя `jWorkPlace`; финальное имя + купленный домен уточняются.
- [ ] **Git-remote** — пока `git init` локально; origin добавим, когда будет GitHub-репо.
