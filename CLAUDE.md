...
| как работает, что делает, где используется, почему, зачем, объясни, разберись, изучи, исследуй, сравни, как устроено, откуда | **P7 Research** |
| латентност, память, RAM, медленно, оптимизаци | **+⚡Perf** (модификатор) |
| задеплой, прод, jwork.jorchik.com, certbot | **+🚀Deploy** (модификатор) |

Приоритет при конфликте: **P5 > P4 > P3 > P2 > P1 > P6 > P7**. Research — самый низкий: любой намёк на
действие/запись (исправь, добавь, поправь конфиг, задеплой) перебивает его на action-профиль. Research
включается, только когда цель запроса — **понять/объяснить**, а не изменить. Дальше — **режим autonomous-run по
...
**P7 — Research (вопрос по коду → структурированный ответ).** `Scope → Investigate → Synthesize → Report → Done`
— **read-only: ничего не меняет, ничего не исполняет.** Без Plan Mode.
> **Граница с Explore (не путать):** `Explore` **локализует** (где/что определено/вызывается) — один
> поисковый агент, выход = дайджест-**вход в стадию**. `P7` **объясняет и судит** (как устроено / почему
> так / чем отличается / чем грозит) — **консилиум экспертов**, выход = grounded-ответ **человеку**.
> Вопрос уровня «где/что» → зови просто `Explore`-агент, профиль не разворачивай (оверинжиниринг).
```
Scope→Investigate  Investigate→Synthesize  Investigate→Scope (вопрос сузился/расширился)
Synthesize→Investigate (пробел в понимании)  Synthesize→Report  Report→Done
```
| Стадия | Агент(ы) | Режим |
|---|---|---|
| Scope | оркестратор: разбить вопрос на углы, выбрать релевантные домены (по картам, **не** открывая код) | → |
| Investigate | **консилиум read-only** по углам вопроса: `Explore` (fan-out — call sites, конвенции, где определено/используется) + доменные эксперты `architect` / `rag-indexing-engineer` / `llm-engineer` / `performance-engineer` / `security-auditor` (все без Edit/Write) | П |
| Synthesize | оркестратор: свести в grounded-ответ (цитаты `file:line`), развести расхождения консилиума, пометить неясное | → |
| Report | структурированный ответ: **TL;DR → разбор по пунктам с `file:line` → поток/диаграмма если уместно → «неясности / за рамками»** | → |

**Возможности (что дать):** `Read`/`Grep`/`Glob`, агент `Explore`, `ast-index` CLI, read-only персона-эксперты;
Bash **только read-only-навигация** (`git log`/`blame`/`show`, `ls`, `grep`/`rg`); `WebSearch`/`WebFetch` —
опционально, лишь для внешней доки библиотек, приоритет всегда — код репо.
**Запрещено (жёстко):** ❌ `Edit`/`Write`/`NotebookEdit` — ноль правок файлов; ❌ developer/qa-агенты на
запись, `commit`/`push`/PR/deploy, стадия Ship; ❌ **выполнение кода проекта и прогон тестов ради
результата** (тесты читаем как источник истины, не гоняем). 🔁 **Эскалация (как в P5):** если вопрос
перерос в «а теперь исправь/сделай» — Research завершается на Report и **порождает новую задачу** в
P1/P2/P6 с находками как входом, сам код не трогает. **Grounding-инвариант продукта — на себя:** ответ
обоснован цитатами `file:line`; чего в коде не нашли — явное «за рамками / не установлено», без догадок
и отката на общие знания. 🔒 Security-gate действует: вопрос про токены/секреты/промпты Слоя B → в
консилиум обязателен `security-auditor`.
...
промптах) и эскалируют только security/необратимое/противоречие инвариантам. **P6/P7 — тоже без Plan
Mode** (правка доки / read-only исследование): гейт качества у них не план, а Verify (P6) / grounded-цитаты (P7).
