...
> ⚠️ **Проект в стадии «фундамент».** Кода ещё нет — этот файл задаёт правила, инварианты,
> агентов и роадмап ДО первой строки. По мере реализации разделы уточняются. Открытые
> решения собраны в конце (`## Открытые решения`) — не выдумывай их молча, спрашивай.
...
- Claude Code крутится на **VPS jorchik.com**. **RAM 3.8 ГБ** → локальные Ollama-модели **≤3B**;
  тяжёлое — облачный API. **Ollama уже поднят на `:11434`** (в т.ч. `nomic-embed` для эмбеддингов).
- Деплой — паттерн соседей: **systemd + nginx + certbot**, секреты в env/`.env`.
...
- **Домен:** `jwork.jorchik.com` — поддомен `jorchik.com` (паттерн как `llm.jorchik.com` у
  `webchat_with_local_llm`). Прописать A-запись + nginx server_name + certbot при деплое.
