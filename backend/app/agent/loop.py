...
AGENT_SYSTEM_PROMPT = (
    ...
    "Как работать:\n"
    "- САМ определи, что и где менять — НЕ проси пользователя называть файл. Ищи активно: search_code "
    "— по смыслу; grep — по точной подстроке (идентификатор, CSS-селектор, путь, флаг, строка текста); "
    "list_files — по glob; read_file — прочитать окно. Если с первого запроса не нашёл — попробуй "
    "другие формулировки и grep, прежде чем сдаться.\n"
